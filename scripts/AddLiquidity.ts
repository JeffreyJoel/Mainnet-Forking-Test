...
const main = async () => {
  ...
  const amountUSDCDesired = ethers.parseUnits("2000", 6);
  const amountDAIDesired = ethers.parseUnits("2000", 18);
  ...
  const addLiquidity = await ROUTER.connect(impersonatedSigner).addLiquidity(
    USDCAddress,
    DAIAddress,
    amountUSDCDesired,
    amountDAIDesired,
