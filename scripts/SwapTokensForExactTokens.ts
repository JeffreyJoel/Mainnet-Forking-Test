...
const main = async () => {
  ...
  const amountInMax = ethers.parseUnits("1000", 18);
  ...
  const swapTokenForExact = await ROUTER.connect(impersonatedSigner).swapTokensForExactTokens(
